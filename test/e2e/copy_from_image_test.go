...
func TestCopyAnImageFromATarToARepoThatDoesNotContainNonDistributableLayersButTheFlagWasIncluded(t *testing.T) {
	...
	t.Run("airgapped environment", func(t *testing.T) {
		...
		if !regexp.MustCompile("Error: file sha256\\-.*\\.tar\\.gz not found in tar\\. hint: This may be because when copying to a tarball, the --include-non-distributable-layers flag should have been provided.").MatchString(stdOutWriter.String()) {
...
func TestCopyAnImageFromARepoToATarThatDoesNotContainNonDistributableLayersButTheFlagWasIncluded(t *testing.T) {
	...
	if strings.Contains(stdOutWriter.String(), "hint: This may be because when copying to a tarball, the --include-non-distributable-layers flag should have been provided") {
