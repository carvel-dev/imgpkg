...
func TestCopyTarSrc(t *testing.T) {
	...
	withResume := []bool{false, true}
	for _, enableResume := range withResume {
		additionalTestName := ""
		if enableResume {
			additionalTestName = " and tar copy is resumed it is able to copy everything to the destination"
		}

		t.Run(fmt.Sprintf("When a bundle contains an ImageIndex%s", additionalTestName), func(t *testing.T) {
			...
			if enableResume {
				imgpkg.Run([]string{"copy", "-b", bundleInfo.RefDigest, "--to-tar", tempBundleTarFile, "--resume"})
			}
		...
		t.Run(fmt.Sprintf("When a bundle contains an ImageIndex that contains an image with a non-distributable layer%s", additionalTestName), func(t *testing.T) {
			env := helpers.BuildEnv(t)
			imgpkg := helpers.Imgpkg{T: t, L: helpers.Logger{}, ImgpkgPath: env.ImgpkgPath}
			defer env.Cleanup()
...
copy |      - %s`, ref.Context().Digest(imageWithNonDistributableLayer.Digest).String(), digest.String())
		...
		t.Run(fmt.Sprintf("When an ImageIndex%s", additionalTestName), func(t *testing.T) {
			env := helpers.BuildEnv(t)
			imgpkg := helpers.Imgpkg{T: t, L: helpers.Logger{}, ImgpkgPath: env.ImgpkgPath}
			defer env.Cleanup()
			...
			if enableResume {
				imgpkg.Run([]string{"copy", "-i", imageIndex.RefDigest, "--to-tar", tempTarFile, "--resume"})
			}
		...
		t.Run(fmt.Sprintf("When a bundle contains an image that no longer exists on the registry%s", additionalTestName), func(t *testing.T) {
			env := helpers.BuildEnv(t)
			imgpkg := helpers.Imgpkg{T: t, L: helpers.Logger{}, ImgpkgPath: env.ImgpkgPath}
			defer env.Cleanup()
		...
		t.Run(fmt.Sprintf("When there is no longer access to the origin registry the copy is successful%s", additionalTestName), func(t *testing.T) {
			env := helpers.BuildEnv(t)
			imgpkg := helpers.Imgpkg{T: t, L: helpers.Logger{}, ImgpkgPath: env.ImgpkgPath}
			defer env.Cleanup()
