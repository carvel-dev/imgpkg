...
`, innerBundleRef)
	...
	t.Run("not collocated bundle pulled by tag is not cacheable", func(t *testing.T) {
		outDir := env.Assets.CreateTempFolder("bundle-annotation")

		subBundleDirectoryPath := strings.ReplaceAll(bundleDigestRef, "sha256:", "sha256-")
		out := imgpkg.Run([]string{"pull", "--recursive", "-b", fmt.Sprintf("%s:%s", outerBundle, outerBundleTag), "-o", outDir, "--output-type=yaml"})
...
- image: %s
  imagesLock:
    path: %s
    updated: false`, outerBundleRef, filepath.Join(outDir, ".imgpkg", "images.yml"), innerBundleRef, filepath.Join(outDir, ".imgpkg", "bundles", subBundleDirectoryPath, ".imgpkg", "images.yml")), out)
	})

	t.Run("not collocated bundle pulled by sha is not cacheable", func(t *testing.T) {
		outDir := env.Assets.CreateTempFolder("bundle-annotation")

		subBundleDirectoryPath := strings.ReplaceAll(bundleDigestRef, "sha256:", "sha256-")
		out := imgpkg.Run([]string{"pull", "--recursive", "-b", outerBundleRef, "-o", outDir, "--output-type=yaml"})
		require.YAMLEq(t, fmt.Sprintf(`cacheable: false
image: %s
imagesLock: 
  path: %s
...
- image: %s
  imagesLock:
	...
	t.Run("collocated bundle pulled by tag is not cacheable", func(t *testing.T) {
		outDir := env.Assets.CreateTempFolder("bundle-annotation")

		subBundleDirectoryPath := strings.ReplaceAll(bundleDigestRef, "sha256:", "sha256-")
		imgpkg.Run([]string{"copy", "-b", fmt.Sprintf("%s:%s", outerBundle, outerBundleTag), "--to-repo", env.RelocationRepo})
		out := imgpkg.Run([]string{"pull", "--recursive", "-b", fmt.Sprintf("%s:%s", env.RelocationRepo, outerBundleTag), "-o", outDir, "--output-type=yaml"})
		require.YAMLEq(t, fmt.Sprintf(`cacheable: false
image: %s
imagesLock: 
  path: %s
...
- image: %s
  imagesLock:
    path: %s
    updated: true`, fmt.Sprintf("%s@%s", env.RelocationRepo, outerBundleDigest), filepath.Join(outDir, ".imgpkg", "images.yml"), fmt.Sprintf("%s@%s", env.RelocationRepo, bundleDigestRef), filepath.Join(outDir, ".imgpkg", "bundles", subBundleDirectoryPath, ".imgpkg", "images.yml")), out)
	})

	t.Run("collocated bundle pulled by sha is cacheable", func(t *testing.T) {
		outDir := env.Assets.CreateTempFolder("bundle-annotation")

		subBundleDirectoryPath := strings.ReplaceAll(bundleDigestRef, "sha256:", "sha256-")
		imgpkg.Run([]string{"copy", "-b", fmt.Sprintf("%s@%s", outerBundle, outerBundleDigest), "--to-repo", env.RelocationRepo})
		out := imgpkg.Run([]string{"pull", "--recursive", "-b", fmt.Sprintf("%s@%s", env.RelocationRepo, outerBundleDigest), "-o", outDir, "--output-type=yaml"})
		require.YAMLEq(t, fmt.Sprintf(`cacheable: true
image: %s
imagesLock: 
  path: %s
  updated: true
